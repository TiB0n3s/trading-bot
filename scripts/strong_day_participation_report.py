...
def compute_session_metrics(bars, threshold_pct):
    ...
    ret_30m = (
        _pct_change(session_open, closes[min(29, len(closes) - 1)]) if len(closes) >= 5 else None
    )
    ret_60m = (
        _pct_change(session_open, closes[min(59, len(closes) - 1)]) if len(closes) >= 5 else None
    )
...
def analyze_symbol(symbol, target_date, threshold_pct):
    ...
    minutes_strong_without_alert = metrics.get("minutes_strong") if status == "no_signals" else None

    max_buy_opportunity_score = max(
        (
            float(r["buy_opportunity_score"])
            for r in buy_rows
            if r["buy_opportunity_score"] is not None
        ),
        default=None,
    )
    ...
    return {
        ...
        **metrics,
        ...
        "auto_buy_max_score": max(
            (float(r["score"]) for r in auto_rows if r["score"] is not None), default=None
        ),
...
def _fmt_blockers(blocker_counts):
    if not blocker_counts:
        return "—"
    return ", ".join(
        f"{cat} x{n}" for cat, n in sorted(blocker_counts.items(), key=lambda x: -x[1])[:4]
    )
...
def _render_affordability_section(strong):
    ...
    print(f"  Symbols with strong days where sizing/capacity gates fired: {len(capacity_hits)}")
    print()
    print(
    ...
    primary_afford = [
        r for r in capacity_hits if r.get("primary_blocker") in {"affordability", "exposure_cap"}
    ]
    if primary_afford:
        print()
        print("  Note: symbols with primary_blocker=affordability were rejected before Claude.")
        print(
            "  The pre-Claude affordability gate now checks whether buying_power can buy at least 1 share at the signal price."
        )
        print("  High-priced symbols (e.g. ASML, AMD, COST) may fail at reduced macro multipliers.")
        print("  To investigate: python3 blocked_signal_outcome_report.py --category affordability")
...
def _render_tv_coverage_section(strong):
    ...
    timely = [r for r in tv_strong if r not in uncovered and r not in late]

    print()
    print("── TradingView Alert Coverage ───────────────────────────────────────────")
    print(f"  TV-alert symbols with strong day : {len(tv_strong)}")
    print(
    ...
    print(f"    No coverage     (no alert received at all)               : {len(uncovered)}")
...
def print_report(results, target_date, min_session_pct):
    ...
    strong = [
        r
        for r in valid
        if r.get("session_return_pct") is not None and r["session_return_pct"] >= min_session_pct
    ]
    ...
    if not strong:
        ...
        print(
            "  (Possible causes: market holiday, weekend, threshold too high, bar data unavailable)"
        )
        if errors:
...
def _print_detail_block(r, min_session_pct):
    ...
    if first_strong:
        print(
            f"    {'first_strong_time':.<34} {first_strong}  ({_fp(first_strong_ret)} at threshold cross)"
        )
    else:
        print(
            f"    {'first_strong_time':.<34} never crossed {min_session_pct:.1f}% threshold intraday (end-of-day only)"
        )

    mins_strong = r.get("minutes_strong_without_alert")
    if mins_strong is not None:
        print(
            f"    {'minutes_strong_without_alert':.<34} {mins_strong} min  (no buy alert received)"
        )
    ...
    print(
        f"    {'auto_buy_first_candidate_time':.<34} {r.get('auto_buy_first_candidate_time') or '—'}"
    )
    ...
    print(
        f"    {'prediction_sample_size':.<34} {r.get('prediction_sample_size') if r.get('prediction_sample_size') is not None else '—'}"
    )
