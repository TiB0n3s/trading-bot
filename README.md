...
- Current auto-buy strategy authority is frozen. The OHLCV-derived auto-buy
  stack has no demonstrated deployable edge and must remain observe/research
  only unless a future thesis clears the promotion checklist in
  `ops/auto_buy_strategy_status.md`: discrimination, decile lift, regime split,
  blocked-null testing, multiple-testing correction, leakage/circularity audit,
  net-of-costs edge, and wider independent-day validation.
- The auto-buy research detector now applies both blocked permutation nulls for
...
- Auto-buy paper execution plumbing can run from internal Alpaca-bar candidates across the approved universe when `AUTO_BUY_SIGNAL_MODE=internal_all` and `AUTO_BUY_LIVE_BUYS=true`, but current strategy status is research-only because the existing OHLCV-derived stack has not demonstrated deployable edge. Candidate capture remains useful for learning and counterfactual review; it is not a reason to loosen gates.
...
- Paper auto-buy has bounded strong-evidence promotion plumbing, but it has not earned new capital authority for the current OHLCV-derived strategy. Do not relax setup, probability, learned-memory, or conviction gates to increase trade count without a fresh evidence note superseding `ops/auto_buy_strategy_status.md`.
- Paper learning authority plumbing exists for paper/dry-run only, but it does not supersede the current auto-buy strategy freeze or grant authority to the unvalidated OHLCV-derived stack. Where enabled for validated paper workflows, it can only act after hard blockers pass, is capped, is logged through the canonical `AuthorityMatrix`, `IntelligenceAdjudicator`, and ordered `GateEngine`, and cannot override stale signals, broker/account constraints, cash-safe/cash-full mode, explicit symbol overrides, macro/regime hard blocks, execution-quality blocks, or Claude infrastructure failures.
