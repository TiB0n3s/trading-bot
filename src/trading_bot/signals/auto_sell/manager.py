...
def evaluate_sell_pressure(
    *,
    ...
    high_water = (
        unrealized_plpc if high_water_plpc is None else max(high_water_plpc, unrealized_plpc)
    )
...
def evaluate_position_momentum(position: Any, session: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if unrealized_plpc >= profit_tier_3_pct and negative_windows >= 2:
    ...
    if (label == "fading" or score <= -3) and m15 < 0 and m30 < 0 and vwap_dist < -0.10:
        return {
            "symbol": symbol,
            "action": "watch",
...
def log_position_momentum_check(
    position, session, decision, auto_sell_enabled=False, order=None
) -> None:
    session = session or {}
    order = order or {}
...
def recently_bought(
    symbol: str, min_hold_minutes: int = MIN_HOLD_MINUTES_BEFORE_AUTO_SELL
) -> tuple[bool, str]:
...
def maybe_execute_auto_sell(position, decision, market_open: bool) -> dict[str, Any] | None:
    ...
    if too_recent:
        logger.warning(f"POSITION MOMENTUM AUTO-SELL skipped for {symbol}: {hold_reason}")
        return None
    ...
    is_trailing_giveback = decision.get(
        "severity"
    ) == "profit_protection" and "trailing_giveback" in str(decision.get("reason", ""))
...
def main() -> int:
    ...
    for _, _, decision in rows:
        ...
        print(
            f"  {decision['symbol']:<6} {decision['action']:<15} {decision['reason']}{pressure_part}"
        )
