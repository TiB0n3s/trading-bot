...
def _score_current_features(features: dict[str, Any], context: dict[str, Any]) -> tuple[float, list[str]]:
    reasons: list[str] = []
...
def _paper_sizing(
    *,
    ...
    current_vol = (
        ...
        or _float(_dict(account_state.get("volatility_normalization")).get("rolling_volatility_20_pct"))
        or atr_pct
    )
...
def build_historical_bar_paper_strategy(
    *,
    ...
    reasons: list[str] = []
    intelligence = historical_bar_intelligence or _dict(
