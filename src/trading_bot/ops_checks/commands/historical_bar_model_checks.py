...
def _artifact_hygiene(candidate_dir: Path, *, stale_days: int) -> dict[str, Any]:
    ...
    stale_binaries = [str(path) for path in binaries if now - path.stat().st_mtime > stale_seconds]
    return {
    ...
    *,
    ...
    for rows in by_label.values():
        ...
        protected_ids.update(
            str(row.get("model_id") or "") for row in rows[: max(1, keep_per_label)]
        )
...
def run_historical_bar_model_readiness(
    *,
    ...
    ok = bool(assessments) and all(
        item.status == "observe_only_candidate_ready" for item in assessments
    )
    print()
    if ok:
