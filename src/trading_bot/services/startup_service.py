...
class StartupService:
    ...
    def run(self) -> None:
        ...
        self._run_step(
            "Session momentum table initialization", self.deps.init_session_momentum_table
        )
    ...
    def _log_ml_authority_config(self) -> None:
        if self.deps.ml_authority_config is None:
            return
        try:
            self.log.info(f"ML authority config at startup: {self.deps.ml_authority_config()}")
    ...
    def _init_recent_favorable_setups(self) -> None:
        ...
        self.deps.prune_recent_favorable_setups(self.deps.recent_favorable_setup_ttl_minutes)
    ...
    def reconcile_positions(self) -> None:
        try:
            ...
            try:
                ...
                self.log.error(f"Startup reconciliation: failed to fetch Alpaca positions: {exc}")
                alpaca_symbols = set()
                alpaca_positions = []
