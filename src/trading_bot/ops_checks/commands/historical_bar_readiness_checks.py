...
def run_historical_bar_readiness(
    *,
    ...
    feature_ready_pct = _pct(len(feature_ready), len(feature_nulls)) if feature_nulls else 100.0
    ...
    print(
        f"invalid_price_rows         : {invalid_price if invalid_price is not None else 'not_scanned'}"
    )
    print(
        f"zero_volume_rows           : {zero_volume if zero_volume is not None else 'not_scanned'}"
    )
    print(f"duplicate_scan             : {quality.get('duplicate_scan', 'skipped')}")
    print(
        f"duplicate_rows             : {duplicate_rows if duplicate_rows is not None else 'not_scanned'}"
    )
