    ...
    out.update(
        {
            ...
            "review_label": (
                "approved_profitable"
                if approved and realized is not None and realized > 0
                else "approved_loss_or_flat"
                if approved
                else "rejected_missed_opportunity"
                if missed
                else "rejected_contained_or_unknown"
            ),
            "focus": ("exit_quality" if approved else "counterfactual_outcome_coverage"),
        }
    )
    return out
    ...
    out.update(
        {
            ...
            "review_label": "needs_candidate_coverage"
            if total == 0
            else "candidate_coverage_available",
        }
    )
    return out
    ...
    out.update(
        {
            ...
            "review_label": (
                "potentially_early_exit"
                if missed_upside is not None and missed_upside > 1.0
                else "drawdown_protection"
    ...
    out.update(
        {
            ...
            "review_label": "promotion_candidate"
            if status in {"size_down_candidate", "narrow_block_candidate"}
            else "not_ready_or_observe",
        }
    )
    return out
    ...
    out.update(
        {
            ...
            "review_label": (
                "clean_setup"
                if quality in {"favorable", "premium"} and failed in {"low", "unknown"}
    ...
    *,
    ...
    return {
        ...
        "exit_pattern": exit_pattern_interpreter(
            (canonical.get("regime_state") or {}) | (canonical.get("exit_state") or {})
        ),
