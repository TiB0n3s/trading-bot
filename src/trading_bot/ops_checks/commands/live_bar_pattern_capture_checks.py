...
def run_live_bar_pattern_capture_report(
    ...
    *,
    ...
    latest_age = _age_minutes(
        summary.get("latest_created_at") or summary.get("latest_bar_timestamp")
    )
    ...
    print(
        f"freshness_ok            : {latest_fresh if session_active else 'not_required_outside_active_session'}"
    )
    ...
    print(
        "  service: SessionMomentumService.refresh_from_bars -> BarPatternFeatureService.persist_features"
    )
