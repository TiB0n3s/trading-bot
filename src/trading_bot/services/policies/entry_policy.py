...
def evaluate_buy_opportunity(
    *,
    ...
    required_confirmations = int(adaptive_buy_confirmation.get("required_buy_confirmations") or 3)
    ...
    try:
        ...
        if aligned_for_buy is False:
            if fast_lane_eligible:
                ...
                required = max(required, 4)
                reasons.append(
                    "raised to 4: market alignment caution plus elevated symbol/setup risk"
                )
            else:
                required = max(required, 3)
                reasons.append(
                    "kept at 3: market alignment caution without elevated symbol/setup risk"
                )
