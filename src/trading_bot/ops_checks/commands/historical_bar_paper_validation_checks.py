...
def _score_rows(
    rows: list[dict[str, Any]], *, label_target: str, threshold: float
) -> dict[str, Any]:
    total = 0
    paper_taken = 0
    paper_winners = 0
...
def build_historical_bar_paper_validation_payload(
    *,
    ...
    sweep = [
        _score_rows(rows, label_target=label_target, threshold=item) for item in tested_thresholds
    ]
    scored = (
        sweep[0]
        if len(sweep) == 1
        else _score_rows(rows, label_target=label_target, threshold=threshold)
    )
...
def build_historical_bar_walk_forward_payload(
    *,
    ...
    negative_delta_folds = sum(
        1 for row in fold_rows if float(row.get("hit_rate_delta") or 0.0) <= 0
    )
