...
| --- | --- | --- | --- | --- |
| `app.py` | Gunicorn/systemd, tests, imports of `process_signal` | `src/trading_bot/web/app_factory.py` plus temporary root shim | Root file below 100 lines, routes/startup/container moved, `/status` and webhook smoke tests pass | Phase 2 in progress: Flask app construction/route registration moved to `src/trading_bot/web/app_factory.py`; startup-service wiring moved to `src/trading_bot/runtime/startup.py`; app-specific runtime settings parsing moved to `src/trading_bot/config/runtime.py`; signal entrypoint wrapper moved to `src/trading_bot/runtime/signal_entrypoint.py` |
...
| `ops_check.py` | Operator CLI, cron, docs | `src/trading_bot/ops_checks/cli.py` plus grouped command registry modules | Root shim delegates only, command registry covers existing commands, docs updated | Phase 3 root shim complete; packaged CLI handler split remains |
| `scripts/*.py` | Legacy cron, operator scripts, compatibility imports | `src/trading_bot/*`, package CLIs, or `ops_check.py` reports | Root has <=5 Python files and scripts are migrated by ownership | Root cleanup complete; staged package migration remains |
| `scripts/auto_buy_manager.py` | Cron/operator CLI | `src/trading_bot/signals/auto_buy/cli.py` | Candidate scoring/execution split, paper smoke test passes, cron updated | Phase 4 |
| `scripts/position_manager.py` | Cron/operator CLI | `src/trading_bot/positions/cli.py` | Exit evaluation/broker actions/rendering split, paper no-position smoke test passes | Phase 4 |
| `scripts/position_momentum_monitor.py` | Cron/operator CLI | `src/trading_bot/positions/momentum_cli.py` | Session momentum/bar capture ownership clarified, cron updated | Phase 4 |
...
- Done: `src/trading_bot/config/runtime.py` owns app-specific runtime settings
  parsing while root `app.py` explicitly publishes compatibility globals.
- Keep root `app.py` as compatibility shim/runtime context until startup,
  runtime callbacks, and systemd/Gunicorn callers are migrated.
