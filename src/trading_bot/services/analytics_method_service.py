...
def build_analytics_method_state(
    *,
    ...
    policy_artifacts = _dict(
        account_state.get("policy_artifacts") or account_state.get("policy_artifact_status")
    )
    ...
    historical_bar_model_intelligence = (
        _dict(account_state.get("historical_bar_model_intelligence"))
        or build_historical_bar_model_intelligence()
    )
    historical_bar_paper_strategy = (
        _dict(account_state.get("historical_bar_paper_strategy"))
    ...
    analytics_families = {
        ...
        "paper_strategy_ensemble": {
            ...
            "master_confidence_score": historical_bar_paper_strategy.get("master_confidence_score"),
        },
        "risk_analytics": {
    ...
    gaps = [
        name for name, item in analytics_families.items() if item.get("status") == "not_integrated"
    ]
    ...
    ai_pattern_payload = _compact_ai_pattern(
        ai_momentum_pattern
        or deterministic_momentum_pattern(
            symbol=symbol,
            action=context.get("action") or account_state.get("action"),
            regime_state={
            ...
            momentum_state={
                ...
                "session_momentum_30m_pct": (
                    context.get("session_momentum_30m_pct") or session.get("momentum_30m_pct")
                ),
            },
            trend_state={
    ...
    ai_review_suite = existing_ai_review_suite or build_ai_review_suite(
        ...
        canonical={
            ...
            "setup_state": {
                ...
                "failed_breakout_risk": _dict(setup_quality.get("structure")).get(
                    "failed_breakout_risk"
                ),
            },
            "regime_state": {
                "exit_pressure_state": _dict(account_state.get("exit_decision_quality")).get(
                    "exit_pressure_state"
                ),
            },
        },
