...
def run_lifecycle_analysis(
    ...
    *,
    ...
    print(
        f"  {'time':<25} {'sym':<6} {'act':<4} {'status':<33} {'exit':<18} {'pnl':>8} {'rej60':>8}"
    )
    print(f"  {'-' * 25} {'-' * 6} {'-' * 4} {'-' * 33} {'-' * 18} {'-' * 8} {'-' * 8}")
    ...
    matched_without_snapshot = payload.summary.get("approved_matched_exit_missing_snapshot", 0)
    ...
    if open_or_unlinked:
        print()
        print(
            f"[WARN] {open_or_unlinked} approved row(s) are still open or lack matched exit linkage"
        )
        return False
