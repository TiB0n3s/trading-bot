...
def resolve_range(args):
    ...
    if args.week:
        ...
        return (
            f"WEEK {monday.isoformat()} to {(saturday - timedelta(days=1)).isoformat()}",
            monday.isoformat(),
            saturday.isoformat(),
        )
...
def main():
    ...
    parser.add_argument(
        "--active-only",
        action="store_true",
        help="Detail rows only for symbols with signals/trades",
    )
    ...
    print(
        f"  active rows   : {sum(1 for r in enriched if r['signals'] > 0 or r['closed_trades'] > 0)}"
    )
    print()
    ...
    print_bucket(
        "By Sector Alignment", aggregate(enriched, lambda r: r["sector_alignment"]), args.limit
    )
    print_bucket(
        "By Index Alignment", aggregate(enriched, lambda r: r["index_alignment"]), args.limit
    )
    print_bucket(
        "By Price Location", aggregate(enriched, lambda r: r["price_location"]), args.limit
    )
    print_bucket(
        "By Volume Context", aggregate(enriched, lambda r: r["volume_context"]), args.limit
    )
    print_bucket(
        "By Catalyst Score",
        aggregate(enriched, lambda r: bucket_score(r["catalyst_score"], "cat")),
        args.limit,
    )
    print_bucket(
        "By Relative Strength Score",
        aggregate(enriched, lambda r: bucket_score(r["relative_strength_score"], "rs")),
        args.limit,
    )
    print_bucket(
        "By Daily % Bucket",
        aggregate(enriched, lambda r: bucket_pct(r["daily_pct"], "daily")),
        args.limit,
    )
    print_bucket(
        "By Intraday % Bucket",
        aggregate(enriched, lambda r: bucket_pct(r["intraday_pct"], "intra")),
        args.limit,
    )
    print_bucket(
        "By Supply Chain Risk",
        aggregate(enriched, lambda r: bucket_score(r["supply_chain_risk_score"], "supply")),
        args.limit,
    )
