                ...
                finding["examples"].append(
                    {
                        ...
                        "summary": str(row.get("event_summary") or raw.get("event_summary") or "")[:160],
                    }
                )
    ...
    *,
    ...
    for item in findings:
        ...
        if item["examples"]:
            ...
            print(f"         example         : {example['market_date']} {example['source']} - {example['summary']}")

    if payload.get("truncated"):
        print()
        print(f"[WARN] output truncated to top {len(findings)} symbols; increase --limit to inspect more")
