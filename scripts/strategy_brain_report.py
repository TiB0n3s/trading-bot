...
def section_strategy_memory(strategy_memory):
    ...
    print(
        f"  {'Symbol':<8} {'Rec':<9} {'Trades':>6} {'Expect':>9} {'Win%':>7} {'MinScore':>8} Reason"
    )
    ...
    interesting = top_items(
        ...
        predicate=lambda k, v: (
            v.get("recommendation") in ("avoid", "caution", "favor") or v.get("manual_override")
        ),
        sort_key=lambda k, v: (
            {"avoid": 0, "caution": 1, "favor": 2, "neutral": 3, "observe": 4}.get(
                v.get("recommendation"), 9
            ),
            k,
        ),
        limit=25,
...
def section_missed_opportunity(missed):
    ...
    print(
        f"  {'Category':<28} {'Rec':<18} {'N':>4} {'Missed%':>8} {'GoodRej%':>9} {'Avg30m':>8} Reason"
    )
...
def section_excursion(excursion):
    ...
    print(
        f"  {'Symbol':<8} {'Rec':<16} {'N':>4} {'P&L':>10} {'MFE%':>8} {'MAE%':>8} {'Giveback%':>10} Reason"
    )
...
def section_policy_backtest(policy):
    ...
    print(f"    hard_gate_rejected         : {policy.get('hard_gate_rejected')}")
    print(f"    policy_relevant_rejected   : {policy.get('policy_relevant_rejected')}")
    print(f"    other_rejected             : {policy.get('other_rejected')}")
    ...
    print(
        f"  policy_would_allow_policy_rejects: {policy.get('policy_would_allow_policy_relevant_rejected')}"
    )
...
def section_recommendations(strategy, missed, excursion, policy):
    ...
    if policy and policy.get("recommendation") == "policy_too_strict":
        recommendations.append(
            "Decision policy may be too strict; review approved buys it would have blocked."
        )
    elif policy and policy.get("recommendation") == "policy_too_loose":
        recommendations.append(
            "Decision policy may be too loose; review rejected buys it would have allowed."
        )
    ...
    if missed:
        ...
        for cat, info in cats.items():
            if info.get("recommendation") == "review_too_strict":
                recommendations.append(f"Gate '{cat}' may be too strict: {info.get('reason')}")
