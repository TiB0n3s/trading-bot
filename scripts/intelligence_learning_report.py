...
def aggregate(rows, key_fn):
    ...
    return sorted(
        out, key=lambda x: (x["expectancy"], x["realized_pnl"], x["events"]), reverse=True
    )
...
def print_bucket(title, rows, limit=None):
    ...
    headers = ["Bucket", "Ev", "Sym", "Sig", "Appr%", "Ord", "Fill", "Closed", "Win%", "P&L", "Exp"]
...
def print_top_insights(rows):
    ...
    if not closed:
        ...
        print(
            "  The learner will become useful after several sessions with events and matched trades."
        )
        return
...
def main():
    ...
    print_bucket(
        "Learned by Event Type", aggregate(enriched, lambda r: r["event_type"]), args.limit
    )
    print_bucket(
        "Learned by Expected Market Impact",
        aggregate(enriched, lambda r: r["expected_market_impact"]),
        args.limit,
    )
    print_bucket(
        "Learned by Trade Relevance",
        aggregate(enriched, lambda r: r["trade_relevance"]),
        args.limit,
    )
    print_bucket("Learned by Bias", aggregate(enriched, lambda r: r["bias"]), args.limit)
    print_bucket(
        "Learned by Entry Quality", aggregate(enriched, lambda r: r["entry_quality"]), args.limit
    )
    print_bucket(
        "Learned by Risk Level", aggregate(enriched, lambda r: r["risk_level"]), args.limit
    )
    print_bucket(
        "Learned by Catalyst Score",
        aggregate(enriched, lambda r: bucket(r["catalyst_score"], "cat")),
        args.limit,
    )
    print_bucket(
        "Learned by Consumer Appetite",
        aggregate(enriched, lambda r: bucket(r["consumer_appetite_score"], "demand")),
        args.limit,
    )
    print_bucket(
        "Learned by Profit Potential",
        aggregate(enriched, lambda r: bucket(r["profit_potential_score"], "profit")),
        args.limit,
    )
    print_bucket(
        "Learned by Supply Chain Risk",
        aggregate(enriched, lambda r: bucket(r["supply_chain_risk_score"], "supply")),
        args.limit,
    )
    print_bucket(
        "Learned by Competitive Risk",
        aggregate(enriched, lambda r: bucket(r["competitive_risk_score"], "comp")),
        args.limit,
    )
    ...
    print(
        "  Next step after enough data: persist these learning summaries and feed them into morning context."
    )
