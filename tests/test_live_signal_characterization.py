...
import app as _app
from tests.test_process_signal_rejections import _PRICE, _account, _buy, _Env, _sell
...
def _approved_downstream(**overrides):
    base = {
        ...
        "app.public_ml_authority_config": MagicMock(
            return_value={"layered_model_authority": {"enabled": False}}
        ),
...
def test_second_look_rejection_blocks_order_submission():
    ...
    with _Env(
        **_approved_downstream(
            **{
                "app.evaluate_signal": MagicMock(
                    return_value={"approved": True, "confidence": "high", "position_size_pct": 1.0}
                ),
...
def test_strategy_weak_score_sets_size_cap_before_claude():
    ...
    with _Env(
        **_approved_downstream(
            **{
                "app.evaluate_strategy_observe_only": MagicMock(
                    return_value=_StrategyResult(score=30)
                ),
                "app.evaluate_signal": MagicMock(side_effect=_capture_decision),
            }
        )
...
def test_buy_opportunity_cap_reduces_submitted_size():
    place_order = MagicMock(return_value={"order_id": "order-2", "status": "submitted", "qty": 1})
    with _Env(
        **_approved_downstream(
            **{
            ...
            **_approved_downstream(
                **{
                    ...
                    "app._try_portfolio_rotation": MagicMock(
                        return_value=(
                            True,
                            "submitted rotation sell",
                            {"weakest": {"symbol": "MSFT"}},
                        )
                    ),
                    "app.evaluate_signal": MagicMock(
                        return_value={"approved": False, "confidence": "medium", "reason": "stop"}
                    ),
                }
            )
        ) as env:
