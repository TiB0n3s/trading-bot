...
def _summary_from_context_matches(matches):
    ...
    learned_min_scores = [
        int(m["min_setup_score"]) for m in matches if isinstance(m.get("min_setup_score"), int)
    ]
    return worst_context_rec, learned_min_scores


def _pattern_recommendation_rank(recommendation: str | None) -> int:
    order = {
        "avoid": 4,
        "caution": 3,
        "neutral": 2,
    ...
    return order.get(recommendation, 0)


def _bar_pattern_lookup_keys(
    signal_context: dict[str, Any] | None,
) -> tuple[str | None, str | None]:
    ctx = signal_context if isinstance(signal_context, dict) else {}
    features = _as_dict(
        ctx.get("bar_pattern_features")
        or ctx.get("latest_bar_pattern_features")
        or ctx.get("historical_bar_features"),
        "bar_pattern_features",
    )
    label = (
        features.get("pattern_label")
        or features.get("bar_pattern_label")
        or ctx.get("pattern_label")
        or ctx.get("bar_pattern_label")
    )
    opportunity_quality = (
        features.get("opportunity_quality")
        or features.get("bar_opportunity_quality")
        or ctx.get("opportunity_quality")
        or ctx.get("bar_opportunity_quality")
    )
    opportunity_action = (
        features.get("opportunity_action")
        or features.get("bar_opportunity_action")
        or ctx.get("opportunity_action")
        or ctx.get("bar_opportunity_action")
    )
    opportunity_key = None
    if opportunity_action and opportunity_quality:
        opportunity_key = f"{opportunity_action}|{opportunity_quality}"
    elif opportunity_quality:
        opportunity_key = str(opportunity_quality)
    return (str(label) if label else None, opportunity_key)


def _bar_pattern_evidence_for_symbol(
    symbol: str,
    mem: dict[str, Any],
    signal_context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Return learned EFI/PVT pattern memory for the current live/historical bar."""
    ...
    for value in symbol_label_context.values():
        if isinstance(value, dict):
            try:
                ...
                pass
    label_key, opportunity_key = _bar_pattern_lookup_keys(signal_context)
    active_matches = []
    if label_key and isinstance(symbol_label_context.get(label_key), dict):
        active_matches.append(
            {
                "label": "bar_pattern_label",
                "key": label_key,
                **symbol_label_context[label_key],
            }
        )
    if opportunity_key and isinstance(symbol_opportunity_context.get(opportunity_key), dict):
        active_matches.append(
            {
                "label": "bar_pattern_opportunity",
                "key": opportunity_key,
                **symbol_opportunity_context[opportunity_key],
            }
        )
    ready_matches = [m for m in active_matches if bool(m.get("authority_ready"))]
    active_recommendations = [str(m.get("recommendation") or "observe") for m in ready_matches]
    active_recommendation = (
        max(active_recommendations, key=_pattern_recommendation_rank)
    ...
    return {
        ...
        "runtime_effect": (
            "paper_pattern_learning_authority"
            if ready_matches
            else mem.get(
        ...
        "authority_ready": bool(ready_matches),
        "active_recommendation": active_recommendation,
        "min_setup_score": max(min_scores) if min_scores else None,
        "matched_pattern_label": label_key,
        "matched_opportunity_key": opportunity_key,
...
def _apply_bar_pattern_authority(
    *,
    recommendation: str,
    min_setup_score: int | None,
    bar_pattern_evidence: dict[str, Any],
) -> tuple[str, int | None]:
    if not bar_pattern_evidence.get("authority_ready"):
        return recommendation, min_setup_score
    pattern_rec = str(bar_pattern_evidence.get("active_recommendation") or "observe")
    if pattern_rec in {"avoid", "caution"}:
        recommendation = pattern_rec
    elif pattern_rec == "favor" and recommendation in {"observe", "neutral", "none"}:
        recommendation = "favor"
    pattern_min = bar_pattern_evidence.get("min_setup_score")
    if isinstance(pattern_min, int):
        min_setup_score = max(
            [score for score in (min_setup_score, pattern_min) if isinstance(score, int)]
        )
    return recommendation, min_setup_score
...
def memory_for_signal(symbol, signal_context=None):
    ...
    lookup_context = normalize_strategy_memory_context(signal_context).to_lookup_context()
    if isinstance(signal_context, dict):
        for key in (
            "bar_pattern_features",
            "latest_bar_pattern_features",
            "historical_bar_features",
        ):
            if key in signal_context:
                lookup_context[key] = signal_context[key]
    ...
    if not symbol_mem:
        ...
        bar_pattern_evidence = _bar_pattern_evidence_for_symbol(symbol, mem, signal_context)
        recommendation, min_setup_score = _apply_bar_pattern_authority(
    ...
    if worst_context_rec in ("avoid", "caution"):
        ...
        rec = "favor"
    bar_pattern_evidence = _bar_pattern_evidence_for_symbol(symbol, mem, signal_context)
    rec, min_score = _apply_bar_pattern_authority(
        recommendation=rec,
...
def contextual_memory_for_signal(symbol, intelligence_context=None, memory_override=None):
    ...
    bar_pattern_evidence = _bar_pattern_evidence_for_symbol(symbol, mem, ctx)
    for match in bar_pattern_evidence.get("active_matches") or []:
        if not match.get("authority_ready"):
            continue
        matches.append(
            {
                ...
                "min_setup_score": match.get("min_setup_score"),
                "expectancy": match.get("avg_forward_return_pct"),
                "win_rate_pct": match.get("best_buy_window_rate_pct"),
                "trades": match.get("forward_outcome_rows") or match.get("rows"),
                "reason": match.get("reason"),
                "manual_override": False,
                "runtime_effect": match.get("runtime_effect"),
                "evidence_label": match.get("evidence_label"),
            }
        )
