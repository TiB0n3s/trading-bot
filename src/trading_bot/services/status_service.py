...
def build_status_payload(runtime):
    ...
    result["portfolio_rotation"] = {
        ...
        in ("1", "true", "yes", "on"),
        "require_replace_now": os.getenv("PORTFOLIO_REPLACEMENT_REQUIRE_REPLACE_NOW", "true")
        .strip()
        .lower()
        in ("1", "true", "yes", "on"),
        ...
        "runtime_effect": "live_sell_path"
        if os.getenv("PORTFOLIO_REPLACEMENT_LIVE_SELLS", "false").strip().lower()
        in ("1", "true", "yes", "on")
        else "observe_only",
    }
            ...
            if ctx_path.exists():
                ...
                result["risk_macro_policy_compare"] = {
                    ...
                    "matches_live": (
                        ...
                        and macro_policy_compare.get("risk_multiplier")
                        == macro_live.get("risk_multiplier")
                        and macro_policy_compare.get("max_new_positions")
                        == macro_live.get("max_new_positions")
                        and macro_policy_compare.get("block_new_buys")
                        == macro_live.get("block_new_buys")
                    ),
                }
            else:
                result["risk_macro_policy_compare"] = {
    ...
    try:
        ...
        bias_avoid = sorted(
            sym
            for sym, entry in _market_bias_state().items()
