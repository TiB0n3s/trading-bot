...
def _bucket(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "avg_distance_from_vwap": rounded(avg([safe_float(r.get("distance_from_vwap")) for r in rows])),
...
def _recommendation(
    *,
    ...
    if avg_fwd15 <= -0.08 and short_windows >= long_windows * 1.2:
        return "favor_short_or_sell_rallies", f"negative avg_fwd15={avg_fwd15:.3f}% with more short windows"
    if long_windows >= 40 and short_windows >= 40:
        return "two_sided_timing_required", "both long and short windows appeared; require local confirmation"
        ...
        *,
        ...
        for symbol, sym_rows in sorted(by_symbol.items()):
            ...
            symbol_memory[symbol] = {
                ...
                "avg_ret_fwd_30m": rounded(avg([safe_float(r.get("ret_fwd_30m")) for r in labeled])),
                ...
                "max_short_state_score": max((short_state_score(r)[0] for r in sym_rows), default=0),
        ...
        setup_memory = {
            setup: _bucket(setup_rows)
            for setup, setup_rows in sorted(by_setup.items())
        }
        time_bucket_memory = {
            bucket: _bucket(bucket_rows)
            for bucket, bucket_rows in sorted(by_time_bucket.items())
        }

        top_long_windows = sorted(
            all_best_longs,
            key=lambda r: safe_float(r.get("ret_fwd_30m"), safe_float(r.get("ret_fwd_15m"), -999.0)) or -999.0,
            reverse=True,
        )[:25]
