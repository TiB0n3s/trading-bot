...
def summarize_replacement_intelligence(rows, pos_rows):
    ...
    for r in strong_candidates:
        ...
        enriched.append(
            (edge if edge is not None else -999, r, weakest, weakest_plpc, weakest_pressure)
        )
...
def summarize_recent_samples(buy_rows, pos_rows):
    ...
    high_buys = sorted(
        scored_buys, key=lambda r: float(get(r, "buy_opportunity_score") or 0), reverse=True
    )[:10]
    ...
    high_pressure = sorted(
        pressure_rows, key=lambda r: float(get(r, "sell_pressure_score") or 0), reverse=True
    )[:10]
