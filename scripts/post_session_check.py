...
def rebuild_matches():
    ...
    try:
        ...
        ok(
            f"matched_trades rebuilt; matched={len(matched)} open_symbols={sum(1 for lots in open_lots.values() if lots)}"
        )
        return True
    except Exception as e:
...
def main():
    ...
    checks.append(
        run_cmd("Filter Report", [sys.executable, "filter_report.py", "--date", target_date])
    )
    checks.append(
        run_cmd("Position Review", [sys.executable, "position_review.py"], critical=False)
    )
    checks.append(run_cmd("Drawdown Report", [sys.executable, "drawdown_report.py", target_date]))
    checks.append(
        run_cmd("Analytics Report", [sys.executable, "analytics_report.py", "--date", target_date])
    )
    checks.append(
        run_cmd(
            "Rejected Outcome Builder",
            [sys.executable, "rejected_signal_outcome_builder.py", "--date", target_date],
        )
    )
    checks.append(
        run_cmd(
            "Rejected Outcome Validation",
            [sys.executable, "ops_check.py", "rejected-outcomes", target_date],
        )
    )
    checks.append(
        run_cmd(
            "Strong-Day Participation",
            [
                sys.executable,
    ...
    checks.append(
        run_cmd(
            ...
            [sys.executable, "prediction_validation_report.py", "--date", target_date],
        )
    )
