...
def summarize_day(day_bars):
    ...
    day["regular_return_pct"] = (
        safe_round(pct_change(reg["close"], reg["open"])) if reg else None
    )
...
class RollingMomentumService:
    ...
    def build_symbol_context(self, symbol: str) -> dict[str, Any]:
        ...
        summaries = {
            day: summarize_day(day_bars)
            for day, day_bars in sorted(bars_by_date.items())
        }

        market_days = [
            day for day, summary in summaries.items()
        ...
        premarket_return = pct_change(today_pre["close"], prior_close) if prior_close and today_pre else None
        ...
        current_session_return = (
            pct_change(latest_price, today_reg["open"]) if today_reg else None
        )
