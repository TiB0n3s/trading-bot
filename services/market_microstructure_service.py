...
def classify_market_microstructure(
    *,
    ...
    open_low = _float(
        opening.get("low")
        or intraday.get("opening_range_low")
        or session.get("opening_range_low")
    )
    first_5_return = _float(intraday.get("first_5m_return_pct") or session.get("first_5m_return_pct"))
    first_15_return = _float(intraday.get("first_15m_return_pct") or session.get("first_15m_return_pct"))
    first_30_return = _float(intraday.get("first_30m_return_pct") or session.get("first_30m_return_pct"))
    ...
    volume_surge_ratio = _float(momentum.get("volume_surge_ratio") or tape.get("volume_surge_ratio"))
    realized_vol = _float(
    ...
    wick_ratio = _float(
        intraday.get("wick_ratio")
        or session.get("wick_ratio")
        or rolling.get("wick_ratio")
    )
    ...
    if volume_state in {"thin", "low"} or (volume_surge_ratio is not None and volume_surge_ratio <= 0.55):
        liquidity_state = "liquidity_vacuum"
        score -= 0.10
        expectancy_modifier -= 0.12
        reasons.append("liquidity_vacuum")
    elif volume_state in {"elevated", "surge"} or (volume_surge_ratio is not None and volume_surge_ratio >= 1.5):
