...
def train_transformer_authority_model(
    *,
    ...
    y_train = torch.tensor([1.0 if value > 0 else 0.0 for value in labels[:split]], dtype=torch.float32)
    x_test = torch.tensor(features[split:], dtype=torch.float32)
    y_test = torch.tensor([1.0 if value > 0 else 0.0 for value in labels[split:]], dtype=torch.float32)
...
def score_transformer_authority_artifact(
    *,
    ...
    try:
        ...
        model.eval()
        vector = torch.tensor([[_float(features.get(col)) for col in feature_columns]], dtype=torch.float32)
        with torch.no_grad():
...
def evaluate_transformer_authority(
    *,
    ...
    enabled = str(env.get("TRANSFORMER_AUTHORITY_ENABLED", "false")).lower() in {"1", "true", "yes", "on"}
    ...
    max_age = int(env.get("TRANSFORMER_MODEL_MAX_AGE_SECONDS") or env.get("ML_MODEL_MAX_AGE_SECONDS") or 0)
    base = {
        "version": TRANSFORMER_AUTHORITY_VERSION,
    ...
    if status not in TRANSFORMER_LIVE_STATUSES:
        return {**base, "status": status, "reason": "registry status does not grant transformer authority"}
    ...
    if guard.get("fallback_required"):
        return {**base, "status": status, "staleness_guard": guard, "reason": "model staleness guard requires fallback"}
    features = (
    ...
    if not score.get("scored"):
        return {**base, "status": status, "staleness_guard": guard, "score": score, "reason": score.get("reason")}
    ...
    if probability < block_threshold and mode in {"paper_gate", "live_candidate", "warn_only"}:
        decision = "block"
        size_multiplier = 0.0
        reason = f"transformer probability {probability:.3f} below block threshold {block_threshold:.3f}"
    elif probability < size_down_threshold and mode in {"paper_soft", "paper_gate", "live_candidate", "warn_only"}:
        decision = "size_down"
