...
def summarize(results):
    ...
    if not by_category:
        ...
        print(
            f"  {'Category':<28} {'N':>4} {'Avg30m%':>9} {'AvgMFE%':>9} {'AvgMAE%':>9} {'MissedGood':>10}"
        )
    ...
    if not by_symbol:
        ...
        print(
            f"  {'Symbol':<8} {'N':>4} {'Avg30m%':>9} {'AvgMFE%':>9} {'AvgMAE%':>9} {'MissedGood':>10}"
        )
...
def build_missed_opportunity_memory(results, target_date):
    ...
    memory = {
        ...
        "category_memory": {k: bucket(v) for k, v in sorted(by_category.items())},
...
def main():
    ...
    parser.add_argument(
        "--write-memory", action="store_true", help="Write missed_opportunity_memory.json"
    )
