...
class PreMarketResearchService:
    ...
    def get_recent_bars(self, symbol: str) -> dict[str, Any]:
        ...
        if self.config.fetch_daily_bars:
            try:
                daily_start = (now - timedelta(days=self.config.daily_lookback_days)).isoformat()
        ...
        if out["last_price"]:
            ...
            out["support_levels"] = self.unique_price_levels(supports + [last_price * 0.99])
            out["resistance_levels"] = self.unique_price_levels(resistances + [last_price * 1.01])
