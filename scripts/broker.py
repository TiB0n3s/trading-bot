...
def place_order(
    ...
    try:
        ...
        if is_cash_mode() and not LIVE_TRADING_ENABLED:
            ...
            _record_order_failure(f"live_guard:{EXECUTION_MODE}_requires_live_trading_enabled")
            return None
        account = get_account()
        if not account:
        ...
        if side == "sell":
            try:
                ...
                logger.error(
                    f"Failed to fetch position for {symbol}: {_classify_broker_exception(e)}",
                    exc_info=True,
                )
            ...
            if qty <= 0:
                logger.error(
                    f"Refusing sell for {symbol}: position qty={qty} is {'short' if qty < 0 else 'zero'}, not a long to close"
                )
            ...
            try:
                ...
                for o in open_orders:
                    ...
                    logger.info(
                        f"Cancelled open order {o.id} ({o.side} {o.qty} {symbol} type={o.order_type}) before sell"
                    )
                ...
                if available_qty < qty:
                    ...
                    _record_order_failure(f"sell_qty_mismatch_after_cancel:{available_qty}<{qty}")
                    return None
                ...
                logger.error(
                    f"Failed to cancel open orders for {symbol}: {_classify_broker_exception(e)}",
                    exc_info=True,
                )
            ...
            if risk_level == "very_high" and qty >= 2:
                ...
                qty = qty // 2
                logger.info(
                    f"Risk multiplier applied to {symbol}: very_high risk_level — sizing halved {original_qty} → {qty}"
                )
            ...
            logger.info(
                f"Buy sizing: {symbol} qty={qty} at {current_price} | risk_amount={risk_amount:.2f} balance={balance}"
            )
            if qty < 1:
                logger.error(
                    f"Position size too small for {symbol} - qty rounds to 0 at price {current_price} with balance {balance}"
                )
            ...
            if is_cash_mode():
                ...
                if EXECUTION_POLICY_MODE == "compare":
                    try:
                        ...
                        logger.warning(
                            f"EXECUTION_POLICY_COMPARE cash_cap failed for {symbol}: {e}"
                        )
        ...
        if side == "buy":
            ...
            logger.info(
                f"SELL order - Stop: {stop_price} (above entry), Target: {take_price} (below entry)"
            )
        ...
        logger.info(
            ...
            + (
                f" | Target: {take_price}"
                if take_price is not None
                else " | No TP (position_manager exits)"
            )
        )
        return {
            "order_id": order.id,
