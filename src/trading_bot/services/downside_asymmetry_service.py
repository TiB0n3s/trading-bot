...
def evaluate_downside_asymmetry(
    *,
    ...
    gap_down_pct = _float(
        risk.get("gap_down_vulnerability_pct") or account_state.get("gap_down_vulnerability_pct")
    )
    earnings_days = _float(risk.get("earnings_days") or account_state.get("earnings_days"))
    catalyst_proximity = _label(
        risk.get("catalyst_proximity") or account_state.get("catalyst_proximity")
    )
    overnight_hold = bool(risk.get("overnight_hold") or account_state.get("overnight_hold"))
    short_interest_pct = _float(
        risk.get("short_interest_pct") or account_state.get("short_interest_pct")
    )
    ...
    beta_shock_loss_pct = _float(
        risk.get("beta_shock_loss_pct") or account_state.get("beta_shock_loss_pct")
    )
    ...
    failure_pattern = _label(
        risk.get("failure_pattern_signature") or setup.get("failure_pattern_signature")
    )
    ...
    if failure_pattern:
        ...
        if any(
            token in failure_pattern for token in ("failed_breakout", "gap_reject", "vwap_loss")
        ):
            score += 0.09
            modifier += 0.08
