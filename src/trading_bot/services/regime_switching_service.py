...
def _state_to_canonical_regime_map(means: Any) -> dict[str, int]:
    ...
    if len(rows) != 3:
        ...
        return {
            str(hmm_state): canonical for canonical, (hmm_state, _, _) in enumerate(sorted_states)
        }
...
def _smoothed(
    regime_history: list[int], required_hits: int, window: int
) -> tuple[int | None, bool]:
...
def train_hmm_regime_model(
    *,
    ...
    if artifact_path:
        ...
        joblib.dump(
            {"model": model, "metadata": metadata, "state_to_regime_map": state_to_regime}, path
        )
    ...
    *,
    ...
    try:
        ...
        return RegimeObservation(
            ...
            reasons=[f"artifact_load_failed={exc}", "falling_back_to_deterministic"]
            + fallback.reasons,
        )
    ...
    try:
        ...
        return RegimeObservation(
            ...
            stable=fallback.stable,
            runtime_effect=fallback.runtime_effect,
            reasons=[f"hmm_predict_failed={exc}", "falling_back_to_deterministic"]
            + fallback.reasons,
        )
