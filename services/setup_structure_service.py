...
def evaluate_setup_structure(snapshot: dict[str, Any] | None) -> SetupStructureScore:
    ...
    distance_to_resistance_pct = _float(
        snapshot.get("distance_to_resistance_pct")
        or snapshot.get("next_supply_distance_pct")
    )
    distance_to_support_pct = _float(
        snapshot.get("distance_to_support_pct")
        or snapshot.get("nearest_support_distance_pct")
    )
