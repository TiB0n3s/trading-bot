...
def _candidate_status() -> dict[str, Any]:
    ...
    return {
        ...
        "ready_labels": sorted(
            label
            for label, item in assessed.items()
...
def run_operator_intelligence_dashboard(
    *,
    ...
    required_ok = (
        bool(market_context.exists())
        and len(symbols) >= 20
        and len(model_status["ready_labels"]) >= 2
    )
    print()
    print("=" * 72)
    ...
    print(
        f"  python3 ops_check.py historical-bar-paper-validation 2024-06-01 --end-date {target_date}"
    )
