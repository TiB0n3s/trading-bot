...
def summarize(results):
    ...
    if not by_symbol:
        ...
        print(
            f"  {'Symbol':<8} {'N':>4} {'P&L':>10} {'AvgMFE%':>9} {'AvgMAE%':>9} {'Giveback%':>10}"
        )
        print(f"  {'-' * 8} {'-' * 4} {'-' * 10} {'-' * 9} {'-' * 9} {'-' * 10}")

        for sym, rows in sorted(
            by_symbol.items(), key=lambda x: sum(r["realized_pnl"] for r in x[1])
        ):
            pnl = sum(r["realized_pnl"] for r in rows)
            print(
                f"  {sym:<8} {len(rows):>4} "
    ...
    if not by_setup:
        ...
        print(
            f"  {'Setup':<28} {'N':>4} {'P&L':>10} {'AvgMFE%':>9} {'AvgMAE%':>9} {'Giveback%':>10}"
        )
        print(f"  {'-' * 28} {'-' * 4} {'-' * 10} {'-' * 9} {'-' * 9} {'-' * 10}")

        for setup, rows in sorted(
            by_setup.items(), key=lambda x: sum(r["realized_pnl"] for r in x[1])
        ):
            pnl = sum(r["realized_pnl"] for r in rows)
            print(
...
def print_trade_samples(results, limit=20):
    ...
    givebacks = [r for r in valid if r.get("profit_giveback_pct") is not None]
    ...
    bad_entries = [
        r
        for r in valid
        if r.get("excursion_classification")
        in ("bad_entry_never_worked", "large_adverse_excursion")
    ]
    ...
    if not bad_entries:
        ...
        print(
            f"  {'Exit time':<19} {'Sym':<6} {'P&L':>9} {'MFE%':>8} {'MAE%':>8} {'Class':<24} Setup"
        )
...
def build_excursion_memory(results, target_date=None):
    ...
    def bucket(rows):
        ...
        bad_entries = sum(
            1 for r in rows if r.get("excursion_classification") == "bad_entry_never_worked"
        )
        winner_losers = sum(
            1 for r in rows if r.get("excursion_classification") == "winner_became_loser"
        )
    ...
    return {
        ...
        "symbol_memory": {k: bucket(v) for k, v in sorted(by_symbol.items())},
        "setup_memory": {k: bucket(v) for k, v in sorted(by_setup.items())},
        "classification_memory": {k: bucket(v) for k, v in sorted(by_class.items())},
    }
