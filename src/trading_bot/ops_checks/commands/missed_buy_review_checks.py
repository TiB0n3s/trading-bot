...
def run_missed_buy_review(
    ...
    *,
    ...
    print(
        f"  rows_with_forward_outcome         : {summary['rows_with_forward_outcome']} ({_pct(summary['forward_outcome_coverage_rate'])})"
    )
    ...
    print(
        f"  correctly_avoided_or_bad_candidates: {summary['correctly_avoided_or_bad_candidates']}"
    )
    print(
        f"  missed_good_rate                  : {_pct(summary['missed_good_rate_of_non_taken_with_forward'])}"
    )
    ...
    if payload.top_missed:
        ...
        for item in payload.top_missed[:samples]:
            ...
            promotion_marker = (
                " paper_promotion_review" if item.get("paper_promotion_review_candidate") else ""
            )
            print(
