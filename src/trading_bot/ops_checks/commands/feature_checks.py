...
def run_feature_health(target_date: str, *, base_dir: Path) -> bool:
    ...
    print("Scripts")
    for script in (
        "run_live_features.sh",
        "run_label_features.sh",
        "live_features.py",
        "label_features.py",
    ):
    ...
    print(
        f"  feature_snapshots       {rows['n']:>8}  {rows['min_ts'] or '-'} -> {rows['max_ts'] or '-'}"
    )

    label_rows = label_repo.label_summary()
    print(
        f"  labeled_setups          {label_rows['n']:>8}  {label_rows['min_ts'] or '-'} -> {label_rows['max_ts'] or '-'}"
    )
    ...
    print(
        "  Rotated logs can prove the jobs worked before the rebuild, but they cannot restore rows by themselves."
    )
    print(
        "  A fresh Tuesday session should create feature_snapshots first, then labeled_setups after the 35-minute label delay."
    )
...
def run_feature_watch(target_date: str, *, base_dir: Path) -> bool:
    ...
    if not live_repo.table_exists("feature_snapshots") or not label_repo.table_exists(
        "labeled_setups"
    ):
