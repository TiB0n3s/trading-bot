...
def render(symbols: list[str] | None, history_symbol: str | None, limit: int):
    ...
    session = (
        repo.latest_by_symbol("session_momentum", "updated_at", symbols) if session_cols else {}
    )
    feature = (
        repo.latest_by_symbol("feature_snapshots", "timestamp", symbols) if feature_cols else {}
    )
