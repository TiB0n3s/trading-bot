...
def _print_row(name: str, latest_at: Any, rows: int | None, threshold: int, *, now: datetime) -> bool:
    ...
    print(f"  {name:<28} status={status:<8} age={age_s:>8} rows={rows_s:>6} latest={latest_at or '-'}")
...
def run_context_freshness(target_date: str, *, base_dir: Path) -> bool:
    ...
    if market_context_path.exists():
        mtime = datetime.fromtimestamp(market_context_path.stat().st_mtime, timezone.utc).isoformat()
        ok = _print_row(
            "market_context_file",
            mtime,
            1,
    ...
    print()
    print("[OK] context freshness looks usable" if ok else "[WARN] context freshness has stale or missing inputs")
    return ok
...
def run_data_freshness_gate(target_date: str, *, base_dir: Path) -> bool:
    ...
    print("gate_scope              : market_context,event_context,predictions,setup_snapshots,session_momentum,rolling_momentum,labels")
