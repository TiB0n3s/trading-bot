...
class RolloutThresholds:
    ...
    block_candidate_allowlist: tuple[str, ...] = ("portfolio_decision",)
...
def _restrictions(
    *,
    ...
    if status == RolloutStatus.NARROW_BLOCK_CANDIDATE:
        ...
        setup_scope = [
            item.get("bucket") for item in interactions.get("setup_label", []) if item.get("bucket")
        ][:3]
        regime_scope = [
            item.get("bucket") for item in interactions.get("regime", []) if item.get("bucket")
        ][:3]
        session_scope = [
            item.get("bucket")
...
def assess_feature_family_rollout(
    *,
    ...
    stable_for_block = (
        stability_share is not None and stability_share >= thresholds.min_stability_share_block
    )
    if (
        status == RolloutStatus.SIZE_DOWN_CANDIDATE
        and block_allowed
        ...
        and fn_for_block
        and (
            calibration_error is None or calibration_error <= thresholds.max_calibration_error_block
        )
    ):
