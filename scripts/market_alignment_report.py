...
def gate_readiness(rows, freshness):
    ...
    weak_benchmarks = [
        r for r in rows if r.get("benchmark_trend") in ("neutral/weak", "-/weak", "None/None")
    ]
...
def main():
    ...
    print("  trend_source      : bot signal history, not live index market data")
    ...
    print(
        "Do not promote to a hard gate unless context_freshness is fresh and gate_readiness is reviewed."
    )
