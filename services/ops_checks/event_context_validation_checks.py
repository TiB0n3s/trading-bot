...
def run_event_context_validation(target_date: str, *, base_dir: Path) -> bool:
    ...
    for row in rows:
        ...
        peripheral = bool(raw.get("peripheral_context")) or raw.get("search_scope") == "company_peripheral"
    ...
    ok = issue_rate < 25.0
    print()
    print("[OK] event context validation passed review threshold" if ok else "[WARN] event context validation found inference/source issues")
    return ok
