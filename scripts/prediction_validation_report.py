...
def render_strong_day_buckets(predictions, strong_days) -> None:
    ...
    if not strong_days:
        print(
            "No strong_day_participation rows yet. Run strong_day_participation_report.py --write-db after the session."
        )
        return
    ...
    for bucket in ("high_55_plus", "mid_50_55", "low_45_50", "weak_below_45", "unknown"):
        ...
        participated = sum(
            ...
            if r["primary_status"]
            in ("full_participation", "partial_participation", "auto_buy_participation")
        )
        missed = sum(
            1
            for r in strong_rows
