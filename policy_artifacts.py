...
def _state_hash(files: dict[str, Any]) -> str:
    payload = {
        name: item.get("sha256") if isinstance(item, dict) else item
...
def register_policy_artifact_set(
    ...
    *,
    ...
    registry = _load_json(paths["registry"], {"version": "policy_artifact_registry_v1", "entries": []})
...
def main() -> int:
    ...
    if args.command == "register":
        print(json.dumps(register_policy_artifact_set(
            args.base_dir,
            label=args.label,
            source=args.source,
    ...
    if args.command == "rollback":
        print(json.dumps(rollback_policy_artifacts(
            args.base_dir,
