...
def _write_manifest(base_dir: Path, *, errors=None, stamp: str = "20260605T120000Z") -> None:
...
def test_completion_assessment_not_ready_with_manifest_error():
    ...
    assert payload["readiness"]["latest_manifest_errors"] == 1


def test_completion_assessment_ready_when_only_older_manifest_has_errors():
    with tempfile.TemporaryDirectory() as tmp:
        base_dir = Path(tmp)
        _write_chunk(base_dir, "AAPL", "2026-01-01", "2026-01-05")
        _write_manifest(base_dir, errors=["timeout"], stamp="20260605T120000Z")
        _write_manifest(base_dir, errors=[], stamp="20260606T120000Z")
        old_base = hook.BASE_DIR
        try:
            hook.BASE_DIR = base_dir
            args = type(
    ...
    assert payload["readiness"]["recent_manifest_errors"] == 1
    assert payload["readiness"]["latest_manifest_errors"] == 0
...
def main():
    tests = [
        ...
        test_completion_assessment_ready_when_only_older_manifest_has_errors,
