...
def build_report(report_date: str, late_minutes: int, thin_count: int) -> list[dict]:
    ...
    for symbol in TRADINGVIEW_ALERT_SYMBOLS_LIST:
        ...
        report.append({
            ...
            "first_regular_session_alert": first_regular.strftime("%Y-%m-%d %H:%M:%S %Z") if first_regular else "",
...
def render(report_date: str, report: list[dict]) -> None:
    ...
    print("=" * 132)
    print(f"{'Symbol':<8} {'Alerts':>6} {'First regular':<24} {'MinAfterOpen':>12} {'Status':<22} Notes")
    print("-" * 132)
