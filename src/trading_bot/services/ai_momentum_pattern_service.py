...
def deterministic_momentum_pattern(
    *,
    ...
    if breakout_quality in {"weak_breakout", "liquidity_vacuum_breakout"} or (
        session_phase in {"midday", "lunch"} and volume_state in {"thin", "normal"}
    ):
...
def normalize_ai_momentum_pattern(
    ...
    *,
    ...
    historical_bucket = _dict(raw.get("historical_bucket")) or _dict(
        fallback.get("historical_bucket")
    )
    ...
    return {
        ...
        "expected_horizon": _safe_str(
            raw.get("expected_horizon")
            or fallback.get("expected_horizon")
            or forecast["expected_horizon"]
        ),
        ...
        "holding_time_decay": _safe_str(
            raw.get("holding_time_decay")
            or fallback.get("holding_time_decay")
            or forecast["holding_time_decay"]
        ),
        "confidence": confidence,
        ...
        "missing_evidence": _as_list(
            raw.get("missing_evidence") or fallback.get("missing_evidence")
        ),
        "rationale": rationale,
    }
