...
def run_intelligence_summary(target_date: str, *, base_dir: Path) -> bool:
    ...
    if (
        ...
        print(
            "[WARN] latest event row is newer than daily_symbol_context; run apply_event_scores.py"
        )
    ...
    if (
        ...
        print(
            "[WARN] latest context row is newer than daily_symbol_predictions; run predict_symbol_outcomes.py"
        )
