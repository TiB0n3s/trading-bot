...
def find_matching_outcome(signal, outcomes_by_symbol, entry_window_minutes=90, exit_window_minutes=90):
...
def classify_learning(signal, outcome, match_kind, entry_delay, exit_delay):
    ...
    decision = signal["decision_summary"]
    ...
    if action == "buy":
        if pnl is not None and pnl > 0:
            if entry_delay is not None and entry_delay > 30:
                return "profitable_but_late_entry", "Buy signal eventually matched a profitable trade, but actual entry was delayed over 30 minutes."
            return "profitable_entry", "Buy signal matched a profitable reconstructed trade."

        if pnl is not None and pnl < 0:
            if entry_delay is not None and entry_delay <= 2:
                return "immediate_entry_lost", "Immediate buy entry matched a losing reconstructed trade."
    ...
    if action == "sell":
        if pnl is not None and pnl > 0:
            return "profitable_exit_context", "Sell signal matched exit of a profitable reconstructed trade."
        if pnl is not None and pnl < 0:
...
def build_row(signal, outcome, match_kind):
    ...
    learning_label, learning_reason = classify_learning(signal, outcome, match_kind, entry_delay, exit_delay)
...
def main():
    ...
    parser.add_argument("--replace", action="store_true", help="Clear historical_signal_outcomes before insert")
    ...
    print()
    print(f"  {'Date':<10} {'Sym':<6} {'Act':<5} {'SigTime':<19} {'EntryDelay':>10} {'ExitDelay':>9} {'P&L':>9} {'EntryLabel':<18} {'Learning':<24}")
    print(f"  {'-'*10} {'-'*6} {'-'*5} {'-'*19} {'-'*10} {'-'*9} {'-'*9} {'-'*18} {'-'*24}")
