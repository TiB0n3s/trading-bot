...
def main() -> int:
    ...
    parser.add_argument("--closes", type=int, default=60, metavar="N",
                        help="Number of SPY closes to pull from feature_snapshots (default 60)")
    parser.add_argument("--artifact", default=str(DEFAULT_ARTIFACT),
                        help="Path to trained HMM joblib artifact")
    parser.add_argument("--state", default=str(DEFAULT_STATE),
                        help="Path to persisted regime history JSON")
    parser.add_argument("--lockout-path", default=str(DEFAULT_LOCKOUT),
                        help="Path to persistent lockout JSON")
    parser.add_argument("--available-cash", type=float, default=0.0,
                        help="Available cash for tranche plan preview")
    parser.add_argument("--routing-matrix", action="store_true",
                        help="Print the full routing matrix and exit")
    parser.add_argument("--json", action="store_true",
                        help="Output raw JSON instead of formatted text")
    parser.add_argument("--no-save", action="store_true",
                        help="Do not persist the inferred regime observation")
    ...
    if artifact_path.exists():
        obs = infer_regime_from_artifact(closes=closes, artifact_path=artifact_path, regime_history=history)
        source = "hmm_artifact"
    else:
    ...
    crash = crash_risk_protocol(regime_history=extended_history, lockout_active=lockout_state.active)
