...
def _setup_source_from_snapshot(row) -> str:
    canonical = (
        _load_json(row["canonical_intelligence_json"])
        if "canonical_intelligence_json" in row.keys()
        else {}
    )
...
def run_setup_breakdown(target_date: str, *, base_dir: Path) -> bool:
    ...
    if not rows:
        ...
        print(f"  {'-' * 28} {'-' * 7} {'-' * 8} {'-' * 8}")
        for r in rows:
            print(f"  {r['action']:<28} {r['signals']:>7} {r['approved']:>8} {r['rejected']:>8}")
    ...
    if pnl_rows:
        ...
        print(f"  {'action':<28} {'trades':>6} {'wins':>5} {'avg_pnl%':>9} {'total_pnl%':>10}")
        print(f"  {'-' * 28} {'-' * 6} {'-' * 5} {'-' * 9} {'-' * 10}")
        for r in pnl_rows:
            print(
    ...
    if approved_unknown:
        ...
        print(f"  {'symbol':<8} {'action':<8} {'pnl%':>7} {'won':>4} {'hold_min':>9}  reason")
    ...
    if bucket_signal_rows:
        ...
        for r in bucket_signal_rows:
            ...
            note = (
                "ACTIVE gate"
                if bucket == "weak_below_45"
                else "observe only"
                if bucket in ("low_45_50", "mid_50_55")
                else "tie-breaker"
                if bucket == "high_55_plus"
                else ""
            )
            print(
    ...
    if capture_rows:
        ...
        for r in capture_rows:
            ...
            avg_cap_s = (
                f"{r['avg_capture']:>8.3f}" if r["avg_capture"] is not None else f"{EM_DASH:>8}"
            )
            print(
