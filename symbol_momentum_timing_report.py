...
def print_setup_summary(memory: dict, limit: int) -> None:
    ...
    for setup, payload in sorted(rows, key=lambda item: item[1].get("rows", 0), reverse=True)[:limit]:
        print(
            f"  {setup:<38} "
            f"{payload.get('rows', 0):>5} "
            f"{_fmt(payload.get('avg_ret_fwd_15m'))} "
...
def main() -> int:
    ...
    print_windows("Best Hindsight Short/Sell Windows", memory.get("top_short_windows") or [], args.samples)
