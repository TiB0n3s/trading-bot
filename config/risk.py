...
class RiskConfig:
    ...
    def __post_init__(self) -> None:
        _check(
            ...
            self.macro_position_count_floor, "must be >= 0",
        )
        _check(
            self.portfolio_rotation_min_candidate_score >= 1,
            "portfolio_rotation_min_candidate_score",
            "PORTFOLIO_ROTATION_MIN_CANDIDATE_SCORE",
            self.portfolio_rotation_min_candidate_score, "must be >= 1",
        )
        _check(
            self.portfolio_rotation_max_per_day >= 1,
            "portfolio_rotation_max_per_day", "PORTFOLIO_ROTATION_MAX_PER_DAY",
            self.portfolio_rotation_max_per_day, "must be >= 1",
        )
        _check(
            self.portfolio_rotation_min_hold_minutes >= 1,
            "portfolio_rotation_min_hold_minutes", "PORTFOLIO_ROTATION_MIN_HOLD_MINUTES",
            self.portfolio_rotation_min_hold_minutes, "must be >= 1",
        )
        _check(
            self.portfolio_replacement_mode in _VALID_REPLACEMENT_MODES,
            "portfolio_replacement_mode", "PORTFOLIO_REPLACEMENT_MODE",
        ...
        _check(
            ...
            "PORTFOLIO_REPLACEMENT_MIN_CANDIDATE_SCORE",
            self.portfolio_replacement_min_candidate_score, "must be >= 0",
        )
        _check(
            self.portfolio_replacement_min_buy_score >= 0,
            "portfolio_replacement_min_buy_score", "PORTFOLIO_REPLACEMENT_MIN_BUY_SCORE",
            self.portfolio_replacement_min_buy_score, "must be >= 0",
        )
        _check(
            self.portfolio_replacement_weak_holding_plpc <= 0,
            "portfolio_replacement_weak_holding_plpc",
            "PORTFOLIO_REPLACEMENT_WEAK_HOLDING_PLPC",
            self.portfolio_replacement_weak_holding_plpc, "must be <= 0",
        )
        _check(
...
def load_risk_config(**overrides) -> RiskConfig:
    ...
    kwargs: dict = dict(
        ...
        portfolio_rotation_min_hold_minutes=env_int(
            "PORTFOLIO_ROTATION_MIN_HOLD_MINUTES", 30
        ),
        portfolio_rotation_max_weak_plpc=env_float(
            "PORTFOLIO_ROTATION_MAX_WEAK_PLPC", 0.0
        ),
        ...
        portfolio_replacement_mode=env_str(
            "PORTFOLIO_REPLACEMENT_MODE", "observe_only"
        ).lower(),
        portfolio_replacement_live_sells=env_bool(
        ...
        portfolio_replacement_min_buy_score=env_float(
            "PORTFOLIO_REPLACEMENT_MIN_BUY_SCORE", 15.0
        ),
