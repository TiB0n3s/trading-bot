...
def main():
    ...
    memory = {
        ...
        "trend_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_trend.items())
        },
        "market_bias_context": {
            k: finalize_bucket(v)
        ...
        "entry_quality_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_entry_quality.items())
        },
        "risk_level_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_risk_level.items())
        },
        "market_bias_effective_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_market_bias_effective.items())
        },
        "fundamental_score_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_fundamental_score.items())
        },
        "session_trend_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_session_trend_label.items())
        },
        "prediction_decision_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_prediction_decision.items())
        },
        "setup_label_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_setup_label.items())
        },
        "setup_policy_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_setup_policy_action.items())
        },
        "buy_opportunity_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_buy_opportunity_recommendation.items())
        },
        "symbol_setup_label_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_symbol_setup_label.items())
        },
        "symbol_prediction_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_symbol_prediction_decision.items())
        },
        "symbol_buy_opportunity_context": {
            k: finalize_bucket(v)
            for k, v in sorted(by_symbol_buy_opportunity.items())
        },
