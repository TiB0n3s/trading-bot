...
def _metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    approved_with_mfe = [
        ...
        if _mfe(row) is not None and _outcome(row) is not None
    ]
    winner_became_loser = [
        row
        for row in approved_with_mfe
        if (_mfe(row) or 0) > 0 and (_outcome(row) or 0) <= 0
    ]
    ...
    for interaction_name, field in interactions:
        grouped = _group(
            ...
            lambda row, field=field: (
                f"{_bucket(row, 'symbol_pattern')} x {_bucket(row, field)}"
            ),
        )
...
def _quality_warnings(rows: list[dict[str, Any]], pattern_outcomes: list[dict[str, Any]]) -> list[dict[str, Any]]:
    warnings = []
    total = len(rows)
    if not total:
        return [
    ...
    derived = sum(
        1
        for row in rows
        if _bucket(row, "pattern_source") == "derived_from_canonical_sections"
    )
...
def _governance(pattern_outcomes: list[dict[str, Any]], *, min_sample_size: int) -> list[dict[str, Any]]:
    result = []
    for item in pattern_outcomes:
