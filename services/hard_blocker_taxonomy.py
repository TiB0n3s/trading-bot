...
def classify_hard_blocker(category: str | None, reason: str | None = None) -> HardBlockerClassification:
    ...
    elif domain is None and any(token in reason_text for token in ("buying_power", "broker", "account")):
        domain = "broker_or_account_constraint"
    elif domain is None and any(token in reason_text for token in ("max risk", "daily loss", "risk limit")):
        domain = "max_risk"
