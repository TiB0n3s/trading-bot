...
def run_conviction_stack_report(target_date: str, *, base_dir: Path) -> bool:
    ...
    cap_buckets = [
        ...
        (
            "1.25%+",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and float(r["effective_size_cap_pct"]) >= 1.25
            ),
        ),
        (
            "0.90\u20131.25%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and 0.90 <= float(r["effective_size_cap_pct"]) < 1.25
            ),
        ),
        (
            "0.80\u20130.90%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and 0.80 <= float(r["effective_size_cap_pct"]) < 0.90
            ),
        ),
        (
            "0.75\u20130.80%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and 0.75 <= float(r["effective_size_cap_pct"]) < 0.80
            ),
        ),
        (
            "0.65\u20130.75%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and 0.65 <= float(r["effective_size_cap_pct"]) < 0.75
            ),
        ),
        (
            "0.50\u20130.65%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and 0.50 <= float(r["effective_size_cap_pct"]) < 0.65
            ),
        ),
        (
            "below 0.50%",
            lambda r: (
                r["effective_size_cap_pct"] is not None
                and float(r["effective_size_cap_pct"]) < 0.50
            ),
        ),
    ]
    ...
    for lim, n in limiter_counts.most_common():
        ...
        flag = (
            " \u2190 dominant"
            if capped > 0 and n / max(capped, 1) > 0.40 and lim != "uncapped"
            else ""
        )
    ...
    if capped_rows:
        ...
        print(
            f"  {'dominant_limiter':<26} {'buy_opp':<20} {'setup_action':<12} {'N':>4} {'Appr':>5}"
        )
...
def run_buy_opportunity_report(target_date: str, *, base_dir: Path) -> bool:
    ...
    for r in rows:
        ...
        print(
            f"  {(r['rec'] or EM_DASH):<22} {r['signals']:>8} {(r['approved'] or 0):>5} {pct:>6} {avg_s:>9}"
        )
    ...
    if pnl_rows:
        ...
        for r in pnl_rows:
            ...
            print(
                f"  {(r['rec'] or EM_DASH):<22} {r['exits']:>6} {avg_pnl_s:>8} {win_rate_s:>8} {cap_s:>8}"
            )
    else:
    ...
    if cap_rows:
        ...
        for r in cap_rows:
            print(
                f"  {(r['rec'] or EM_DASH):<22} {(r['dominant_limiter'] or 'uncapped'):<28} {r['n']:>6}"
            )
    ...
    if dc_rows and dc_rows["n"]:
        ...
        print(
            "    No action required \u2014 both signals are independently valid \u2014 but note the overlap."
        )
