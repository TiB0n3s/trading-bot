...
def run_runtime_health(target_date: str, *, base_dir: Path, limit: int = 15) -> bool:
    ...
    skipped_rows = [
        row for row in payload.rows if row.get("lock_acquired") == 0 or row.get("skipped_reason")
    ]
    zero_row_rows = [
        row
        for row in payload.rows
    ...
    if zero_row_rows:
        ...
        for row in zero_row_rows[:limit]:
            print(f"  {row['started_at']} {row['job_name']} duration={_fmt(row['duration_sec'])}")
    ...
    print()
    print(
        "[OK] runtime job ledger is clean"
        if ok
        else "[WARN] runtime job ledger has gaps or failures"
    )
    return ok
...
def run_runtime_health_trend(
    ...
    *,
    ...
    print()
    print(
        "[OK] runtime health trend is clean"
        if payload["clean"]
        else "[WARN] runtime health trend has failures"
    )
