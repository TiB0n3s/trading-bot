...
from repositories.fill_repo import (  # noqa: E402
    trades_missing_confirmed_fills,
    update_trade_fill_by_row_id,
)
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Print what would change without writing to DB"
    )
    ...
    for r in rows:
        ...
        try:
            ...
            print(
                f"  id={r['id']} {r['symbol']:<5} {r['action']:<4} order={oid[:8]}: ERROR querying Alpaca — {e}"
            )
            errors += 1
            continue
        ...
        if status in ("filled", "partially_filled") and filled_avg_price:
            try:
                ...
                print(
                    f"  id={r['id']} {r['symbol']:<5} {r['action']:<4} order={oid[:8]}: SKIP — cannot parse filled_avg_price={filled_avg_price!r}"
                )
                skipped += 1
                continue
            print(
                f"  id={r['id']} {r['symbol']:<5} {r['action']:<4} order={oid[:8]}: APPLY status={status}, fill_price={fp}"
            )
            if not args.dry_run:
            ...
            print(
                f"  id={r['id']} {r['symbol']:<5} {r['action']:<4} order={oid[:8]}: SKIP — status={status} filled_qty={filled_qty} filled_avg_price={filled_avg_price}"
            )
            skipped += 1
