...
def build_advanced_alpha_readiness_payload(
    *,
    ...
    dashboard_path = (
        Path(__file__).resolve().parents[1] / "dashboards" / "friction_heatmap_dashboard.py"
    )
    ...
    items = [
        ...
        _item(
            ...
            checks={
                "bar_order_flow_proxy_available": rows > 0
                and _rate(rows_with_order_flow, rows) >= 0.80,
                "execution_microstructure_available": rows > 0
                and _rate(rows_with_microstructure, rows) >= 0.80,
