...
def evaluate_portfolio_decision(
    *,
    ...
    if candidate_clusters:
        avg_stress_corr = sum(_stress_correlation(c) for c in candidate_clusters) / len(
            candidate_clusters
        )
    ...
    downside_comovement_score = round(
        min(
            1.0,
            (incremental_var_pct / max_incremental_var_pct) if max_incremental_var_pct > 0 else 0.0,
        ),
        4,
    )
    ...
    if crowded_theme or beta_delta > max_beta_delta * 1.5:
        ...
        duplicate_risk_score >= 0.45
        or beta_delta > max_beta_delta
        or incremental_var_pct > max_incremental_var_pct
    ):
        decision = "size_down"
