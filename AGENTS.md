...
(`.Codex/agents/research-analyst.md`), whose system prompt encodes these rules.
...
- Paper learning authority is enabled by default for paper/dry-run only. After
  ...
  Codex low-confidence soft rejection into a capped paper approval when
  ...
  explicit-symbol hard blockers, and never override Codex parse/engine errors.
...
TradingView alert
  ...
  → Codex Haiku decision_engine.py
...
Run deterministic pre-Codex and post-Codex gates through approval services.
...
WHERE market_date = '$TARGET_DATE';
"
Pre-Codex Risk Stack

The app applies a large stack of checks before Codex is called.
...
ApprovalService owns deterministic and Codex/confidence decisions.
