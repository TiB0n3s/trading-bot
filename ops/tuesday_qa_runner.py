...
def due_windows(
    windows: list[CheckWindow], target_date: str, run_due_only: bool
) -> list[CheckWindow]:
    if not run_due_only:
        return windows
...
def main() -> int:
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Print schedule without running checks"
    )
    parser.add_argument(
        "--run-due-only",
        action="store_true",
        help="Run only windows whose scheduled time has already passed",
    )
    parser.add_argument(
        "--no-sleep", action="store_true", help="Run selected windows immediately without waiting"
    )
    ...
    write(
        log_file,
        f"target_date={args.date} dry_run={args.dry_run} run_due_only={args.run_due_only} no_sleep={args.no_sleep}",
    )
