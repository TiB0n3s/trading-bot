...
def _feature_families_from_lifecycle(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [item for item in families if item.get("bucket") not in (None, "", "unknown")]
...
def _runtime_health_input(target_date: str, db_path: Path) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "warnings": (["runtime_health_not_clean"] if not payload.summary.get("clean") else []),
...
def run_ai_intelligence_review(
    ...
    *,
    ...
    event_rows = [dict(row) for row in OpsCheckRepository(db_path).event_source_rows(target_date)]
    ...
    print(f"  policy_conflict       : {suite['policy_disagreement']['conflict_detected']}")
    print(f"  negative_sources      : {suite['policy_disagreement']['negative_sources']}")
    print(f"  candidate_count       : {suite['candidate_universe_review']['candidate_count']}")
    print(f"  near_threshold        : {suite['candidate_universe_review']['near_threshold_count']}")
    print(f"  source_audit          : {suite['source_evidence_audit']['audit_result']}")
    print(f"  operator_briefing     : {suite['daily_operator_briefing']['briefing_label']}")
    print(f"  exit_pattern          : {suite['exit_pattern']['review_label']}")
    print(f"  overlap_risk          : {suite['feature_overlap']['overlap_risk']}")
    print(f"  readiness_status      : {suite['model_readiness']['status']}")
    print(f"  remediation_tasks     : {suite['remediation_tasks']['tasks'][:samples]}")
