...
- `pipeline/after_close_learning.py` runs the recurring after-close quant learning loop: outcome completion, candidate/rejected forward backfill, DuckDB/PyArrow research export, pattern/feature/post-trade/readiness reports, guarded retraining, and observe-only RandomForest/XGBoost model-suite comparison.
