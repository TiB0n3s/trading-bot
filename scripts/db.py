    ...
    with get_connection(db_path) as con:
        con.execute(
            ...
            CREATE TABLE IF NOT EXISTS feature_snapshots (
                ...
                bar_timeframe TEXT,
                bar_count INTEGER,
                bar_contract_version TEXT,
                bar_required_fields TEXT,
                setup_label TEXT,
...
def ensure_decision_snapshots_table(db_path: Path | str = DB_PATH) -> None:
    ...
    migration = next(m for m in MIGRATIONS if m.migration_id == "20260526_005_decision_snapshots")
    ...
    feature_parity_migration = next(
        m for m in MIGRATIONS if m.migration_id == "20260531_016_decision_snapshot_feature_parity"
    )
    canonical_intelligence_migration = next(
        m for m in MIGRATIONS if m.migration_id == "20260531_017_canonical_intelligence_snapshot"
    )
    long_horizon_migration = next(
        m for m in MIGRATIONS if m.migration_id == "20260601_021_long_horizon_session_momentum"
    )
    gate_trace_migration = next(
        m for m in MIGRATIONS if m.migration_id == "20260609_023_decision_snapshot_gate_trace"
    )
        ...
        for statement in long_horizon_migration.statements:
            ...
            if len(alter) >= 6 and alter[2] == "decision_snapshots":
                ...
                if column not in existing_cols:
                    con.execute(statement)
                    existing_cols.add(column)
        for statement in gate_trace_migration.statements:
            alter = statement.strip().split()
            if len(alter) >= 6 and alter[2] == "decision_snapshots":
                column = alter[5]
                if column not in existing_cols:
...
def db_health_summary(db_path: Path | str = DB_PATH) -> dict:
    ...
    with get_connection(db_path) as con:
        ...
        if "trades" in tables:
            summary["trades_count"] = con.execute("SELECT COUNT(*) AS n FROM trades").fetchone()[
                "n"
            ]
