...
def estimate_probabilistic_edge(
    *,
    ...
    if trend_weight is not None and trend_weight >= 1.15 and momentum.get("direction") == "rising":
        probability += 0.04
    ...
    if participation_modifier is not None:
        ...
        if participation_modifier < 1.0:
            ...
            expected_adverse_excursion_pct = round(
                max(
                    0.05,
                    expected_adverse_excursion_pct - (participation_modifier - 1.0) * 0.18,
                ),
                4,
            )
    if volatility_modifier is not None:
        ...
        if volatility_modifier < 1.0:
            ...
            expected_adverse_excursion_pct = round(
                max(
                    0.05,
                    expected_adverse_excursion_pct - (volatility_modifier - 1.0) * 0.18,
                ),
                4,
            )
