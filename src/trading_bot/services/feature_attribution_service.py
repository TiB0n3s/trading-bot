...
def _bucket_metrics(
    *,
    ...
    return {
        ...
        **metrics,
        "hit_rate_delta": (
            round(hit - base_hit, 4) if hit is not None and base_hit is not None else None
        ),
        "ev_delta_pct": (
            round(ev - base_ev, 4) if ev is not None and base_ev is not None else None
        ),
        "mfe_delta_pct": (
            round(mfe - base_mfe, 4) if mfe is not None and base_mfe is not None else None
        ),
        "mae_delta_pct": (
            round(mae - base_mae, 4) if mae is not None and base_mae is not None else None
        ),
    ...
    *,
    ...
    def evaluate_windows(windows: list[list[dict[str, Any]]]) -> dict[str, Any]:
        ...
        for window_rows in windows:
            bucket_rows = [row for row in window_rows if _bucket(row, family_path) == best_bucket]
            if not bucket_rows:
                continue
    ...
    shares = [value for value in (daily["share"], rolling["share"]) if value is not None]
    ...
    *,
    ...
    for family, path in FEATURE_FAMILIES.items():
        ...
        default_bucket_rows = sum(
            item["sample_size"] for item in buckets if item["bucket"] in default_bucket_names
        )
        default_bucket_rate = (
            round(default_bucket_rows / len(outcome_rows), 4) if outcome_rows else None
        )
    ...
    guardrails = [
        _rollout_guardrail(family, min_sample_size=min_sample_size) for family in families
    ]
