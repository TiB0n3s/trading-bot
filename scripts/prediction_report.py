...
def main() -> int:
    ...
    date_filters_used = sum(1 for x in (args.date, args.today, args.last_n_days is not None) if x)
    ...
    print(
        f"Win rate 5m        : {overall['win_rate_5m']:.1f}%"
        if overall["win_rate_5m"] is not None
        else "Win rate 5m        : -"
    )
    print(
        f"Win rate 15m       : {overall['win_rate_15m']:.1f}%"
        if overall["win_rate_15m"] is not None
        else "Win rate 15m       : -"
    )
    print(
        f"Win rate 30m       : {overall['win_rate_30m']:.1f}%"
        if overall["win_rate_30m"] is not None
        else "Win rate 30m       : -"
    )
    ...
    by_setup_label = grouped_summary(
        rows,
        lambda r: r["setup_label"] if "setup_label" in r.keys() and r["setup_label"] else "unknown",
        min_samples=args.min_samples,
        horizon=args.horizon,
    )
    ...
    by_bias_vwap = grouped_summary(
        rows,
        lambda r: (
            f"{(r['market_bias'] or 'unknown')}|{bucket_vwap_distance(r['distance_from_vwap'])}"
        ),
        min_samples=args.min_samples,
        horizon=args.horizon,
    )
    ...
    print_trade_table(
        "Live BUYs by Rejection Category", buy_trades_by_rejection_category, args.limit
    )
    print_trade_table("Setup Policy Blocks by Setup Label", buy_blocks_by_setup_label, args.limit)
    print_trade_table(
        "Setup Policy Blocks by Policy Reason", buy_blocks_by_policy_reason, args.limit
    )
    print_leaderboard(
