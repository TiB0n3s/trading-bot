...
def candidate_outcomes(target_date: str, db_path: Path | str = DB_PATH) -> list[dict[str, Any]]:
    ...
    for row in candidates:
        ...
        for minutes in HORIZONS:
            future_price, future_ts = repo.feature_price_at_or_after(
                item["symbol"], item["timestamp"], minutes
            )
...
def _summarize(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        groups[(row.get("signal_source") or "unknown", row.get("decision") or "unknown")].append(
            row
        )
    ...
    for (source, decision), items in sorted(groups.items()):
        ...
        for minutes in HORIZONS:
            vals = [
                r[f"return_{minutes}m"] for r in items if r.get(f"return_{minutes}m") is not None
            ]
...
def render(target_date: str, rows: list[dict[str, Any]], tv_summary: dict[str, Any]) -> bool:
    ...
    ranked = [row for row in rows if row.get("decision") in ("strong_buy_candidate", "watch")]
