...
def _learning_actions(
    *,
    ...
    if coverage_rate is None or coverage_rate < 0.80:
        actions.append(
            "continue candidate-outcome-backfill until forward-outcome coverage is at least 80%"
        )
    if missed_good <= 0:
    ...
    if top_reasons.get("negative_session_downtrend", 0) >= 3:
        actions.append(
            "review negative_session_downtrend false negatives against symbol-level trend recovery"
        )
    if top_reasons.get("below_vwap", 0) >= 3:
        actions.append(
            "review below_vwap rejection when EFI/PVT or forward MFE shows recovery setups"
        )
    if top_reasons.get("setup_avoid", 0) + top_reasons.get("setup_unclassified_transition", 0) >= 3:
        actions.append(
            "review setup-quality avoid/unclassified labels for missed positive-forward candidates"
        )
    if top_reasons.get("strategy_memory_caution_setup_below_min", 0) >= 3:
        actions.append(
            "review strategy-memory caution thresholds for buckets with positive forward outcome"
        )
    ...
    actions.append(
        "compare missed-buy rows with decision-quality approved rows before tuning thresholds"
    )
    return actions
    ...
    *,
            ...
            top_missed.append(
                {
                    ...
                    "paper_promotion_review_candidate": bool(
                        ...
                        and (_float(row.get("score")) or 0.0)
                        >= (_float(row.get("threshold")) or 0.0)
                        and any(token.startswith("setup_avoid") for token in row_reason_tokens)
                    ),
    ...
    summary = {
        ...
        "avg_missed_good_mfe_pct": (
            round(sum(missed_mfe_values) / len(missed_mfe_values), 4) if missed_mfe_values else None
        ),
