...
def check_market_context():
    ...
    if len(low_default) > len(APPROVED_SYMBOLS) // 2:
        warn(
            f"{len(low_default)} symbols are default neutral/low; parser may not have captured full brief"
        )
    else:
...
def check_rolling_momentum():
    ...
    missing = sorted(APPROVED_SYMBOLS - set(symbols))
    errors = [
        sym for sym, entry in symbols.items() if isinstance(entry, dict) and entry.get("error")
    ]
