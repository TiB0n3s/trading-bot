...
def evaluate_exit_decision_quality(
    *,
    ...
    unrealized_pnl_pct = _float(
        exit_inputs.get("unrealized_pnl_pct") or account_state.get("unrealized_pnl_pct")
    )
    holding_minutes = _float(
        exit_inputs.get("holding_minutes") or account_state.get("holding_minutes")
    )
    ...
    if trend_direction in {"bearish", "down"} or (
        trend_slope_change is not None and trend_slope_change <= -0.35
    ):
        trend_deterioration_state = "deteriorating"
        score += 0.16
        reasons.append("trend_deteriorating")
    elif trend_direction in {"bullish", "up"} and (
        trend_slope_change is None or trend_slope_change >= -0.10
    ):
