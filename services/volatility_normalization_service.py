...
def classify_volatility_normalization(
    *,
    ...
    chase_risk = "normal"
    if stretch_state == "extreme_stretch" or (
        move_zscore is not None and abs(move_zscore) >= 2.5
    ):
        chase_risk = "high"
    elif stretch_state == "stretched" or (
        move_zscore is not None and abs(move_zscore) >= 1.5
    ):
        chase_risk = "elevated"
    ...
    return VolatilityNormalizationFeatures(
        ...
        range_percentile=round(range_percentile, 4)
        if range_percentile is not None
        else None,
