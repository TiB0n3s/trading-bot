...
_FUNDAMENTAL_TO_CANONICAL = {
    syn: canon for canon, syns in FUNDAMENTAL_SYNONYMS.items() for syn in syns
}
...
PRIORITY_SUMMARY_PATTERN = re.compile(
    r"\b(risk[-\s]?on|risk[-\s]?off|bullish|bearish|sentiment)\b", re.IGNORECASE
)
GENERIC_SUMMARY_PATTERN = re.compile(
    r"\b(futures|sentiment|market|macro|overall|outlook|fed|cpi)\b", re.IGNORECASE
)
...
def main():
    ...
    parser.add_argument(
        "--date", help="Override market_date (YYYY-MM-DD). Default = next trading day."
    )
    ...
    if not text.strip():
        print(
            "ERROR: no input text provided (stdin was empty and no file path given)",
            file=sys.stderr,
        )
        sys.exit(1)
