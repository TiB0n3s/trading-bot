...
def build_calibration_bucket_payload(
    ...
    *,
    ...
    for bucket, bucket_rows in grouped.items():
        ...
        false_positives = sum(
            1 for row in approved if (_outcome(row) is not None and float(_outcome(row) or 0) <= 0)
        )
        false_negatives = sum(
            1 for row in rejected if (_outcome(row) is not None and float(_outcome(row) or 0) > 0)
        )
