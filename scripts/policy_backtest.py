...
def summarize(results):
    ...
    policy_relevant_rejected = [
        r for r in actual_rejected if r.get("rejection_scope") == "policy_relevant"
    ]
    ...
    would_allow_policy_relevant_rejected = [
        r for r in policy_relevant_rejected if r["policy_decision"] == "allow"
    ]
...
def print_samples(results, limit=25):
    ...
    rows = [r for r in results if r["actual_approved"] and r["policy_decision"] == "block"]
    ...
    rows = [r for r in results if not r["actual_approved"] and r["policy_decision"] == "allow"]
...
def build_policy_backtest_summary(results):
    ...
    by_rejection_category = defaultdict(
        lambda: {"total": 0, "policy_allow": 0, "policy_size_down": 0, "policy_block": 0}
    )
    ...
    allow_policy_reject_rate = len(would_allow_policy_relevant_rejected) / max(policy_reject_n, 1)
...
def main():
    ...
    parser.add_argument(
        "--write-summary", action="store_true", help="Write policy_backtest_summary.json"
    )
