...
class LiveSignalProcessor:
    ...
    def process(
        ...
        current_et = (
            preflight_result.metadata.get("current_et") if preflight_result is not None else None
        )
        existing_position = (
        ...
        *,
        ...
        return RejectionAdapter(
            reject_current_signal=(
                lambda category, reason, level="warning": (
                    self._reject_current_signal(
                        symbol=symbol,
                        action=action,
                        price=price,
                        account_state=account_state,
            ...
            reject_approval_decision=(
                lambda approval, level="warning": (
                    self._reject_approval_decision(
                        symbol=symbol,
                        action=action,
                        price=price,
                        account_state=account_state,
    ...
    def check_cash_safe_gates(self, *, symbol, action, price, account_state, dedupe_key):
        decision = signal_stage_guards.check_cash_safe_gates(
            ...
            max_new_buys_per_symbol_per_day=(self.deps.cash_safe_max_new_buys_per_symbol_per_day),
    ...
    def apply_setup_stage(
        ...
        *,
        ...
        if action == "buy" and setup_obs.get("setup_policy_action") == "error":
            ...
            has_strong_context = deg_trend_dir == "bullish" and deg_trend_str in (
                "confirmed",
                "developing",
            )
        ...
        if action == "buy":
            ...
            prediction_decision = (
                prediction_gate.get("prediction_decision") or prediction_gate.get("decision") or ""
            )
            ...
            if (
                ...
                and session_return_pct
                >= float(os.getenv("LATE_QUOTE_DELAY_MIN_SESSION_RETURN_PCT", "0.75"))
    ...
    def check_sell_discipline(
        self, *, symbol, action, price, account_state, dedupe_key, existing_position
    ):
    ...
    def run_macro_position_gate(self, **kwargs):
        ...
        if outcome.rejected and outcome.approval:
            return StageResult(
                rejected=kwargs["rejection_adapter"].reject_approval_decision(outcome.approval)
            )
        return StageResult()
    ...
    def run_prediction_bias_session_gate(self, **kwargs):
        outcome = run_prediction_session_tape_gates(
            ...
            prediction_soft_avoid_min_sample_size=(self.deps.prediction_soft_avoid_min_sample_size),
    ...
    def run_final_approval_gates(self, **kwargs):
        ...
        return ApprovalGateResult(claude_account_state=outcome.claude_account_state)
