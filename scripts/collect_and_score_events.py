...
def main():
    ...
    parser.add_argument(
        "--no-dedupe",
        action="store_true",
        help="Insert duplicates instead of skipping existing same-day events",
    )
    parser.add_argument(
        "--predict",
        action="store_true",
        help="Generate observe-only symbol predictions after event/context updates",
    )
    parser.add_argument(
    ...
    by_ai_provider = Counter(
        ((e.get("ai_event_context") or {}).get("provider") or "none") for e in new_events
    )
    ...
    if args.apply_context:
        ...
        print(
            f"Applied event aggregates to daily_symbol_context: updated={updated}, skipped={skipped}"
        )
    ...
    if args.predict:
        ...
        if args.apply_context and updated_symbols_by_date:
            ...
            prediction_symbols = [sym for sym in symbols if sym in APPROVED_SYMBOLS]
            for e in new_events:
