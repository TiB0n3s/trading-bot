...
def run_paper_learning_authority_report(target_date: str, *, base_dir: Path) -> bool:
    ...
    lifecycle_payload = LifecycleAnalysisService(LifecycleAnalysisRepository(db_path)).payload(
        start_date=target_date, end_date=target_date
    )
    lifecycle_by_snapshot = {row.get("decision_snapshot_id"): row for row in lifecycle_payload.rows}
    ...
    for key in sorted(
        k for k in counts if k.startswith("lifecycle_") and k != "lifecycle_unlinked"
    ):
        print(f"  {key:<38} {counts[key]:5d}")
    ...
    print(
        f"  avg_realized_pnl                      {_fmt_num(_avg(realized_pnl_values), digits=2)}"
    )
