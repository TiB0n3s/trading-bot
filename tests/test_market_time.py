...
def test_market_hours_opens_at_945_et():
    assert is_market_hours(et_dt(2026, 5, 11, 9, 44)) is False
    assert is_market_hours(et_dt(2026, 5, 11, 9, 45)) is True


def test_market_hours_closes_at_345_et():
    assert is_market_hours(et_dt(2026, 5, 11, 15, 44)) is True
    assert is_market_hours(et_dt(2026, 5, 11, 15, 45)) is False
...
def test_market_time_accepts_utc_datetime_and_converts_to_et():
    utc = pytz.utc.localize(datetime(2026, 5, 11, 13, 45))  # 9:45 AM ET during EDT
    assert is_market_hours(utc) is True


def test_market_time_handles_standard_time_offset():
    # January uses EST / UTC-5. 14:45 UTC is 9:45 AM ET.
    utc = pytz.utc.localize(datetime(2026, 1, 12, 14, 45))
    assert is_market_hours(utc) is True
